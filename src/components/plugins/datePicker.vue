...
<template>
  <div class="vue-datepicker">
    <input @click.stop="show=!show" :value="current | dateFormat" type="text" class="date-input" readonly :placeholder="placeholder" >
    <div v-if="show" class="vue-datepicker-wrap" >
      ...
      <div class="vue-datepicker-content">
        <table>
          ...
          <tbody>
            <tr v-for="week in list" :key="week.index">
              <td v-for="weekday of week" @click="pick(weekday)" :key="weekday.index"
                :class="{
                  'flag': weekday.flag,
        ...
        <div v-if="selectYear" class="vue-date-picker-year-panel">
          <ul ref="year">
            <li v-for="y of years" @click.stop="pickYear(y)" :class="{'active': y == select.year}"  :key="y.index">{{y}}</li>
          </ul>
          <ul ref="month">
            <li v-for= "(m, $index) of months" :key="$index"
        ...
        <div class="clearDate" @click="setEmpty">清空</div>
      </div>
    </div>
  </div>
</template>
<style lang="stylus">
  .vue-datepicker {
    &>input {
      ...
      background:#f9fafd url(../../assets/img/date_icon.png) no-repeat center right
      font-size: 14px;
      outline: none;
      cursor: pointer;
      &:focus {
      ...
      */
      ...
      for (let i = 0; i < lineNumber; i++) {
        if (i === lineNumber - 1 && nextDays > 0 && nextDays !== 7) {
          this.list[lineNumber] = this.numberList(temp, this.currentMonthEndDate)
